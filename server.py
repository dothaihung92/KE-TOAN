...
_HOA_KHONG_DAU = "ABCDEFGHIJKLMNOPQRSTUVWXYZÁÀẢÃẠĂẮẰẲẴẶÂẤẦẨẪẬÉÈẺẼẸÊẾỀỂỄỆÍÌỈĨỊÓÒỎÕỌÔỐỒỔỖỘƠỚỜỞỠỢÚÙỦŨỤƯỨỪỬỮỰÝỲỶỸỴĐ"
_THUONG_KHONG_DAU = "abcdefghijklmnopqrstuvwxyzáàảãạăắằẳẵặâấầẩẫậéèẻẽẹêếềểễệíìỉĩịóòỏõọôốồổỗộơớờởỡợúùủũụưứừửữựýỳỷỹỵđ"

def _dvc_tim_nut(drv, text_can_tim):
    """Tìm (các) nút/link đang HIỂN THỊ trên trang có chứa đúng đoạn chữ
    text_can_tim (không phân biệt hoa/thường, có/không dấu)."""
    xpath = (f"//*[self::button or self::a or self::span][contains("
             f"translate(normalize-space(.),'{_HOA_KHONG_DAU}','{_THUONG_KHONG_DAU}'),"
             f"'{text_can_tim.lower()}')]")
    try:
...
def _dvc_bam_ky_ho_so(drv):
    ...
    đúng chứng thư số — bấm "Tiếp tục" xong là lúc hub CKS cục bộ (chương
    trình RIÊNG ngoài trình duyệt) bật hộp thoại "Xác nhận PIN" — xem
    _dvc_nhap_pin_ky_so() để tự nhập PIN tiếp bước này."""
    import time as _t
    buoc = []

    def _tim_nut(text_can_tim):
        return _dvc_tim_nut(drv, text_can_tim)
...
def _dvc_nhap_pin_ky_so(pin, so_lan_toi_da=2, cho_hien_toi_da=30, cho_dong_toi_da=8):
    ...
    try:
        ...
        return {"ok": False, "khong_thay_hop_thoai": True, "buoc": [],
                "loi": "Chưa cài thư viện pywinauto (cần để tự điều khiển hộp thoại nhập PIN — "
                       "đây là cửa sổ Windows RIÊNG, ngoài trình duyệt) — đóng phần mềm, chạy lại "
                       "start.bat để tự cài, rồi thử lại. Trong lúc chờ, hãy tự nhập PIN bằng tay."}
    import time as _t
    buoc = []
    ...
    for lan in range(1, so_lan_thuc_te + 1):
        ...
        if not dlg:
            ...
            return {"ok": True, "buoc": buoc, "so_lan_da_thu": lan - 1}
        try:
            edits = dlg.descendants(control_type="Edit")
            if not edits:
                return {"ok": False, "buoc": buoc, "so_lan_da_thu": lan - 1,
                        "loi": "Không tìm thấy ô nhập PIN trong hộp thoại"}
            o_pin = edits[0]
            o_pin.set_focus()
            try:
                o_pin.set_edit_text("")
            ...
            for b in dlg.descendants(control_type="Button"):
                try:
                    if "dang nhap" in _khong_dau(b.window_text()):
                        nut_dn = b
                        break
                except Exception:
                    continue
            if not nut_dn:
                return {"ok": False, "buoc": buoc, "so_lan_da_thu": lan - 1,
                        "loi": "Không tìm thấy nút \"Đăng nhập\" trong hộp thoại"}
            nut_dn.click_input()
            buoc.append("Đã bấm \"Đăng nhập\"")
        except Exception as e:
            return {"ok": False, "buoc": buoc, "so_lan_da_thu": lan - 1,
                    "loi": f"Lỗi khi thao tác hộp thoại PIN: {e}"}

        dong = False
        cho_den2 = _t.time() + cho_dong_toi_da
        while _t.time() < cho_den2:
            if not _dvc_tim_hop_thoai_pin(desk):
                dong = True
                break
            _t.sleep(0.5)
        if dong:
            buoc.append("Hộp thoại đã tự đóng — đăng nhập PIN thành công")
            return {"ok": True, "buoc": buoc, "so_lan_da_thu": lan}

        dlg2 = _dvc_tim_hop_thoai_pin(desk)
        thong_bao_loi = ""
        if dlg2:
            try:
                for t in dlg2.descendants(control_type="Text"):
                    tx_goc = t.window_text()
                    if tx_goc and any(k in _khong_dau(tx_goc) for k in _DVC_PIN_TU_KHOA_SAI):
                        thong_bao_loi = tx_goc
                        buoc.append(f"Phát hiện thông báo trong hộp thoại: {tx_goc}")
                        break
            except Exception:
                pass
...
def get_pin_nop_to_khai():
    return {"pin": _get_setting("pin_nop_to_khai", "12345678")}


@app.post("/api/settings/pin-nop-to-khai")
def set_pin_nop_to_khai(body: dict = Body(...)):
    pin = (body.get("pin") or "").strip()
    if not pin:
        raise HTTPException(400, "Mã PIN không được để trống")
    _set_setting("pin_nop_to_khai", pin)
...
def dvc_nop_to_khai(cid: int, body: dict = Body(...)):
    ...
    # Bấm "Tiếp tục" xong (ở bước trên) là lúc hub CKS cục bộ bật hộp thoại
    # Windows "Xác nhận PIN" — tự nhập PIN mặc định rồi tự bấm "Nộp tờ khai"
    # luôn, KHÔNG dừng lại chờ người dùng nữa (theo yêu cầu). An toàn: chỉ
    # thử PIN tối đa 2 lần rồi dừng hẳn (xem _dvc_nhap_pin_ky_so) để tránh
    # khoá token.
    ket_pin = None
    ket_nop = None
    if ket_ky.get("ok"):
...
def dvc_bam_ky_ho_so(phien_id: str):
    ...
    /api/dvc/nop-to-khai) — dùng khi lần đầu KHÔNG tự bấm "Ký hồ sơ" được:
    tự bấm "Ký hồ sơ" → chọn chứng thư số → "Tiếp tục", rồi tự nhập mã PIN
    (hộp thoại Windows riêng) + tự bấm "Nộp tờ khai" luôn (giống hệt luồng
    tự động trong /api/dvc/nop-to-khai)."""
                    ...
                    if fmts and target_dir and invs:
                      try:
                        ...
                        def _tai_1_file(inv):
                            ...
                            # Hóa đơn "Tổng cục thuế đã nhận KHÔNG MÃ" (ttxly=6): không
                            # có file XML để tải (trang Thuế không cấp mã cho loại này).
                            # KHÔNG gọi API chi tiết ở đây — từng thử "lấy luôn chi tiết
                            # ngay lúc tra cứu" nhưng việc bắn thêm hàng trăm request
                            # chi tiết XEN LẪN các request tải file (chạy song song
                            # nhiều luồng) khiến tường lửa trang Thuế coi là tấn công
                            # và CHẶN CẢ IP ở tầng kết nối (trình duyệt cũng không vào
                            # được — ERR_HTTP2_PROTOCOL_ERROR), người dùng đã xác nhận
                            # cứ tra cứu là bị chặn. Chi tiết dòng hàng của loại hóa
                            # đơn này sẽ được lấy ở bước KẾT XUẤT EXCEL (đã có sẵn cơ
                            # chế thử lại kiên nhẫn + phương án dự phòng tổng tiền theo
                            # bảng kê) — một điểm gọi mạng duy nhất, dễ kiểm soát nhịp.
...
def _misa_ghi_ban_hang(cid, database, preview=True, ghi_de=False):
    ...
    try:
        ...
        for r in rows:
            ...
            # mất thông tin tham chiếu, dù không hiện trong ô chuyên dụng.
            mau_ky_hieu_txt = " - ".join(x for x in
                                         (("Mẫu %s" % maus) if maus else "",
                                          ("Ký hiệu %s" % kyhieu) if kyhieu else "") if x)
            dien_giai = (("Bán hàng - %s - HĐ %s%s %s" %
                         (ten_kh_misa, sohd,
...
def _export_htkk_impl(cid: int, ky: str = "", nguoi_ky: str = "", tu: str = "", den: str = "",
    ...
    # ----- Tổng MUA VÀO + BÁN RA: CHỈ đọc từ file Excel "Bảng kê hóa đơn" đã
    # xuất và lưu sẵn (sheet "BK Mua vào"/"BK Bán ra") — ĐÚNG YÊU CẦU: Kết
    # xuất XML KHÔNG BAO GIỜ tự tra cứu/gọi mạng lại/tính lại từ 'invoices'
    # nữa, kể cả khi không đọc được bảng kê — CHỈ dựa vào bảng kê đã lưu.
    # Trước đây khi không tìm/đọc được file bảng kê thì ÂM THẦM rơi xuống
    # tính lại theo cách cũ (từ 'invoices' trong DB + gọi mạng lấy chi tiết)
    # -> XML có thể ra số liệu KHÁC hẳn bảng kê mà không rõ vì sao (đã xác
    # nhận qua dữ liệu thật: dù đã sửa/xuất lại bảng kê đúng và khởi động lại
    # phần mềm, XML vẫn ra đúng số liệu CŨ — chứng tỏ vẫn rơi vào nhánh cũ).
    ...
    if _tu_file_bk_mua is None:
        raise HTTPException(400,
            ...
            f"{den_ngay_kkhai}) từ file bảng kê đã xuất (sheet 'BK Mua vào', "
            f"file BangKe_HoaDon_{mst_cty}...xlsx trong thư mục tải về/thư mục "
            f"lưu của công ty). Hãy bấm 'Xuất Excel tổng hợp' cho ĐÚNG kỳ này "
    ...
    if _tu_file_bk is None:
        raise HTTPException(400,
            ...
            f"{den_ngay_kkhai}) từ file bảng kê đã xuất (sheet 'BK Bán ra', "
            f"file BangKe_HoaDon_{mst_cty}...xlsx trong thư mục tải về/thư mục "
            f"lưu của công ty). Hãy bấm 'Xuất Excel tổng hợp' cho ĐÚNG kỳ này "
            f"rồi mới bấm 'Kết xuất XML' — Kết xuất XML không tự tra cứu/tính "
            f"lại nữa, chỉ đọc đúng số liệu trong file bảng kê đã xuất.")
...
def export_excel(cid: int, luu_ket_xuat: int = 0, tu_ngay: str = "",
    ...
    can_nap = []
    da_co_file_rows = []
    # Hóa đơn "KHÔNG MÃ" (ttxly=6): đã KIỂM CHỨNG THỰC TẾ (response API thật)
    # rằng endpoint get_detail() (/invoices/detail — cùng API dùng cho nút
    # "Xem hóa đơn" trên trang Thuế) VẪN gọi được và trả về dòng hàng với
    # ĐÚNG số tiền/thuế suất — CHỈ RIÊNG trường "tên hàng hóa" bị trang Thuế
    # để trống (không phải lỗi phần mềm, không phải không gọi được mạng).
    # Vẫn cho gọi mạng bình thường ở lượt nhanh đầu (để lấy đúng breakdown
    # theo từng dòng/thuế suất — chính xác hơn cách gộp theo tổng hóa đơn),
    # nhưng KHÔNG đưa vào vòng CHỜ 429 kiên nhẫn ở "LƯỢT CUỐI" phía dưới (nếu
...
def _parse_detail_json(detail):
    ...
    # Hóa đơn KHÔNG MÃ (ttxly=6): đã kiểm chứng thực tế trang Thuế trả về
    # ĐÚNG số tiền/thuế suất từng dòng qua API này, nhưng luôn để TRỐNG tên
    # hàng hóa (giới hạn của TCT, không phải lỗi đọc dữ liệu) — thay bằng ghi
